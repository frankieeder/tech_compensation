...
class StreamlitApp:
    ...
    def focus_area(self):
        with StreamlitQueryParam(
            "focus_area", self.DEFAULT_FOCUS_AREA
        ) as focus_area_qp:
            focus_area_qp.value = st.text_input(
                "Focus Area",
                value=st.query_params.get("focus_area", self.DEFAULT_FOCUS_AREA),
                key="focus_area",
            )
    ...
    def yac(self):
        with (
            StreamlitQueryParamNumber(
                "yac_min", self.DEFAULT_YAC_MIN, type=int
            ) as yac_min_qp,
            StreamlitQueryParamNumber(
                "yac_max", self.DEFAULT_YAC_MAX or self.max_yac, type=int
            ) as yac_max_qp,
        ):
    ...
    def level_index(self):
        with (
            StreamlitQueryParamNumber(
                "level_index_min", self.DEFAULT_LEVEL_INDEX_MIN, type=int
            ) as level_index_min_qp,
            StreamlitQueryParamNumber(
                "level_index_max",
                self.DEFAULT_LEVEL_INDEX_MAX or self.max_level_index,
                type=int,
            ) as level_index_max_qp,
        ):
    ...
    @functools.cached_property
    def companies(self):
        with StreamlitQueryParamJSON(
            self.QUERY_PARAM_COMPANY_GROUPS, self.DEFAULT_COMPANY_GROUPS
        ) as company_groups_qp:
            company_group_companies = sum(
                (COMPANY_GROUPS[group] for group in company_groups_qp.value), []
            )
            ...
            with (
                StreamlitQueryParamJSON(
                    self.QUERY_PARAM_COMPANIES, company_group_companies
                ) as companies_qp,
                StreamlitQueryParamBoolean(
                    self.QUERY_PARAM_INDIVIDUAL_COMPANIES,
                    self.DEFAULT_INDIVIDUAL_COMPANIES,
                ) as individual_companies_qp,
            ):
                individual_companies_qp.value = st.checkbox(
                    "Choose Individual Companies", value=individual_companies_qp.value
                )
    ...
    def filtered_dmas(self):
        with StreamlitQueryParamJSON(
            "filtered_dmas", self.DEFAULT_FILTERED_DMAS
        ) as filtered_dmas_qp:
    ...
    def filter_to_us(self):
        with StreamlitQueryParamBoolean(
            "filter_to_us", self.DEFAULT_FILTER_TO_US
        ) as input_qp:
    ...
    def scatter_plot_trace(self, df, x_col, y_col, color_col):
        hover_template = "<br>".join(
            f"{k}: %{{customdata[{i}]{':,.2f' if k in [COMP_TC_COL, COMP_SALARY_COL, COMP_STOCK_COL, COMP_BONUS_COL] else ''}}}"
    ...
    def scatter_plot(
        ...
        fig.update_layout(
            title_text=f"{title}<br><sup>{subtitle} [N: {len(df)}]<br>{expected_text}<br>&nbsp;</sup>"
        )
        if log_y:
    ...
    def yoe_options(self):
        with (
            StreamlitQueryParamNumber(
                "yoe_quantile", self.DEFAULT_YOE_QUANTILE
            ) as yoe_quantile_qp,
            StreamlitQueryParamBoolean(
                "use_yoe_mean", self.DEFAULT_USE_YOE_MEAN, to_clear=[yoe_quantile_qp]
            ) as use_yoe_mean_qp,
        ):
            with st.expander(f"{YOE_COL} Options", expanded=not use_yoe_mean_qp.value):
                use_yoe_mean_qp.value = st.checkbox(
                    "Use Mean Years of Experience", value=use_yoe_mean_qp.value
                )
                if use_yoe_mean_qp.value:
                    ...
                    yoe_agg_func = lambda s: s.quantile(
                        yoe_quantile_qp.value, interpolation="higher"
                    )
    ...
    def tc_options(self):
        with (
            StreamlitQueryParamNumber(
                "tc_quantile", self.DEFAULT_TC_QUANTILE
            ) as tc_quantile_qp,
            StreamlitQueryParamBoolean(
                "use_tc_mean", self.DEFAULT_USE_TC_MEAN, to_clear=[tc_quantile_qp]
            ) as use_tc_mean_qp,
        ):
            with st.expander(
                f"{COMP_TC_COL} Aggregation Options", expanded=not use_tc_mean_qp.value
            ):
                use_tc_mean_qp.value = st.checkbox(
                    "Use Mean Total Compensation", value=use_tc_mean_qp.value
                )
                if use_tc_mean_qp.value:
                    ...
                    tc_agg_func = lambda s: s.quantile(
                        tc_quantile_qp.value, interpolation="higher"
                    )
    ...
    def total_compensation_by_yoe(self):
        total_compensation_by_yoe = (
            self.filtered_df.groupby(YOE_COL)[COMP_TC_COL]
            .agg(**{COMP_TC_COL: self.tc_agg_func, "Count": "count"})
            .reset_index()
        )
        total_compensation_by_yoe = total_compensation_by_yoe[
            total_compensation_by_yoe["Count"] > 2
        ]
    ...
    def best_levels(self):
        ...
        best_levels = best_levels[self.aggregated_by_level_df[self.lt_yoe_col] > 0]
        ...
        best_levels = best_levels.rename(
            columns={self.lt_yoe_col: f"# Offers w/ {self.lt_yoe_col}"}
        )
        return best_levels
    ...
    def display_plots(self):
        tc_by_offer_tab, aggregated_tab, best_levels_tab = st.tabs(
            ["By Offer", "Aggregated", "Best Levels"]
        )
